...
if __name__ =='__main__':
    ...
    detector = Yolo7(weights='/home/pqbas/projects/LABINM_Robotics_Automation/seedlings-net/weights/yolov7-vseed.pt',
                    data='./yolov7/opt.yaml', 
                    device='cuda:0')
    
    img = cv2.imread('/home/pqbas/projects/LABINM_Robotics_Automation/seedlings-net/gallery/vertical.jpg')
    predictions = detector.predict(img)
    result = detector.plot_prediction(img, predictions)

    cv2.imshow('Horizontal Seedling',result)
    cv2.waitKey(0)
